"""Deja el tipo de trabajo "Alumbrado viento" con su catálogo definitivo.

La configuración de un tipo de trabajo (qué materiales y qué partidas de mano de
obra lo componen) se hace normalmente desde la pantalla de Configuración del
...
Es idempotente y deja el conjunto EXACTO: agrega lo que falta y quita lo que
sobre. Las reglas que autocompletan la mano de obra a partir del material y del
recupero están en la app, en `reglas_liquidacion.dart`.
"""
...
class Command(BaseCommand):
    help = 'Configura el tipo de trabajo "Alumbrado viento" (materiales y mano de obra).'

    @transaction.atomic
    def handle(self, *args, **options):
        try:
            tipo = TipoTrabajo.objects.get(nombre=TIPO_TRABAJO)
        except TipoTrabajo.DoesNotExist:
            self.stderr.write(f'No existe el tipo de trabajo "{TIPO_TRABAJO}".')
            return
        ...
        materiales = list(Material.objects.filter(matricula__in=MATERIALES))
        self._avisar_faltantes("materiales", MATERIALES,
        ...
        partidas = list(ManoDeObra.objects.filter(partida__in=MANO_DE_OBRA))
        self._avisar_faltantes("partidas", MANO_DE_OBRA,
        ...
        self.stdout.write(self.style.SUCCESS(
            f"{TIPO_TRABAJO}: {tipo.materiales.count()} materiales, "
